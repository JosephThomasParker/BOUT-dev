...
const Field3D interp_to(const Field3D &var, CELL_LOC loc)
{
  if(mesh->StaggerGrids && (var.getLocation() != loc)) {
    ...
    if((location == CELL_CENTRE) || (loc == CELL_CENTRE)) {
      ...
      TRACE("interpolation::interp_to::Going between centred and shifted");
...
#pragma omp parallel
{
	...
	//output_info.write("Using interp_to: now need to communicate in X");
	break;
      }
      case CELL_YLOW: {
	if (var.hasYupYdown() && 
...
#pragma omp parallel
  {
	...
	//output_info.write("Using interp_to: now need to communicate in Y");
	break;
      }
      case CELL_ZLOW: {
#pragma omp parallel
